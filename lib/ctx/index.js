...
var Ctx = function (appSettings, path, method, next) {
...
Ctx.prototype.next = function () {
	if (this.isDone)
	{
		this.unhandledError(new InternalError('Attempt to call next() when isDone (possible concurent next() calls)'));
	}
	else
	{
...
Ctx.prototype.logError = function (error) {
