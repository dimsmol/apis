...
Syslog.prototype.log = function (logger, logLevel, msg) {
	...
	msg = this.createMessage(logger, logLevel, severity, msg);
	this.logger.send(msg, severity);
};

Syslog.prototype.createMessage = function (logger, logLevel, severity, msg) {
	var items = ['[ ', logger.name, ' ', logLevel, ' ] ', msg];
	return items.join('');
};
