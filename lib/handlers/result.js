...
Result.prototype.sendError = function (ctx, error) {
	...
	if (error instanceof WebError) {
		...
		// must add Allowed to headers for '405 Method not allowed' responce
		// according to HTTP specs
		if (status == 405 && error.httpAllowedMethods != null && error.httpAllowedMethods.length > 0) {
			headers = headers || {};
			headers.Allow = error.httpAllowedMethods.join(', ');
		}
	}
	else {
		if (isDebug) {
	...
	ctx.clearError();
	this.respond(ctx, status, result, headers);
};

Result.prototype.respond = function (ctx, status, result, opt_headers) {
	ctx.mechanicsCtx.sendResult(status, result, opt_headers);
	ctx.responseSent();
	ctx.next();
};
