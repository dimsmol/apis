...
Mechanics.prototype.onMessage = function(connection, message) {
	...
	var ctx;
	try {
		ctx = this.createCtx(connection, message);
	}
	catch (err) {
		this.logger.logError(err, 'critical');
	}

	if (ctx && ctx.subPath(this.prefix)) {
