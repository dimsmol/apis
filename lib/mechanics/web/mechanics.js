...
var crossDomainEmulation = require('./cross_domain_emulation');
var Cors = require('./cors');

var RequestEntityTooLarge = errors.RequestEntityTooLarge;
var NotFound = errors.NotFound;

var JsonpMechanics = crossDomainEmulation.JsonpMechanics;
var FrameMechanics = crossDomainEmulation.FrameMechanics;
...
var Mechanics = function () {
	...
	this.crossDomainEmulationSettings = null;
...
Mechanics.prototype.unitInit = function (units) {
	...
	this.crossDomainEmulationSettings = webSettings.crossDomainEmulation;
...
Mechanics.prototype.createSubmechanicsIfNeed = function (ctx) {
	var result = null;
	var req = ctx.req;
	if (!this.crossDomainEmulationSettings.disable && req.method == 'GET') {
		var xdomain = ctx.req.query[this.crossDomainEmulationSettings.key];
		var jsonpSettings = this.crossDomainEmulationSettings.jsonp;
		if (!xdomain && !jsonpSettings.disable && req.query[jsonpSettings.callbackKey]) {
			// special case for jsonp
			xdomain = 'jsonp';
		}
		if (xdomain) {
			var settings = this.crossDomainEmulationSettings[xdomain];
...
Mechanics.prototype.sendCrossDomainEmulationResult = function (ctx, contentType, body) {
	this.onBeforeResponse(ctx);
	var res = ctx.res;
	res.statusCode = 200;
...
Mechanics.prototype.sendJsonpResult = function (ctx, result, callback) {
	...
	this.sendCrossDomainEmulationResult(ctx, 'text/javascript', body);
};
...
Mechanics.prototype.sendFrameResult = function (ctx, result) {
	...
	this.sendCrossDomainEmulationResult(ctx, 'text/html', body);
};
